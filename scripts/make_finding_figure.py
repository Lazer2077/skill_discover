...
(b) an omniscient per-episode oracle barely beats the best single fixed scale.
...
D = json.load(open("paper/figures/finding_data.json"))
go2, h1 = D["go2"], D["h1"]
...
axA.set_title("(a) The optimal command scale is nearly episode-independent", fontsize=9.5)
axA.legend(fontsize=8, loc="upper right"); axA.grid(axis="y", alpha=0.25)
axA.text(0.02, 0.90, "one scale wins\n74–78% of episodes", transform=axA.transAxes,
...
axB.axhspan(min(g["per-episode\noracle"], h["per-episode\noracle"]), 1.0,
...
axB.set_title("(b) A per-episode oracle barely beats one fixed scale", fontsize=9.5)
